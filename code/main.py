...
def collate_fn_bilstm(data):
  ...
  data.sort(key=lambda x: x['cap_len'], reverse=True) # pack_padded_sequence 要求要按照序列的长度倒序排列
  words_embed = [x['caps'] for x in data]
  paded_words_embed = rnn_utils.pad_sequence(words_embed, batch_first=True, padding_value=0)
  for i, cur_data in enumerate(data):
    cur_data['caps'] = paded_words_embed[i]
...
if __name__ == "__main__":
  ...
  if cfg.DATASET_NAME in ['birds', 'coco']:
    ...
    from LGIE_datasets import TextDataset
    from trainer_LGIE import condGANTrainer as trainer
  ...
  output_dir = '../output/%s_%s' % \
               (args.cfg_file.split('/')[-1].split('.')[0], timestamp)
  ...
  dataloader = torch.utils.data.DataLoader(
    dataset, batch_size=cfg.TRAIN.BATCH_SIZE, collate_fn=collate_fn,
