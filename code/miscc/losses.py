...
def discriminator_loss(netD, real_imgs, fake_imgs, conditions,
                       real_labels, fake_labels, words_embs=None, cap_lens=None, image_encoder=None, class_ids=None,
                        w_words_embs=None, wrong_caps_len=None, wrong_cls_id=None):
    # Forward
    ...
    for i in range(cfg.TREE.BRANCH_NUM):
        if cfg.TRAIN.W_GAN:
            ...
            if netsD[i].UNCOND_DNET is  not None:
                ...
                g_loss = cond_errG
            errG_total += g_loss * cfg.TRAIN.W_GAN
        ...
        for fea_idx in range(len(real_features)):
            cur_real_features = real_features[fea_idx]
            cur_fake_features = fake_features[fea_idx]
            feature_loss += F.mse_loss(cur_real_features, cur_fake_features)


    feature_loss = feature_loss / 3. * cfg.TRAIN.W_VGG
    errG_total += feature_loss
    logs += 'feature_loss: %.2f ' % feature_loss

    l1_loss = l1_loss * cfg.TRAIN.W_L1
    errG_total = errG_total + l1_loss
    logs += 'l1_loss: %.2f ' % l1_loss
