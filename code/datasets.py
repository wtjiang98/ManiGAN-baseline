...
def prepare_data_LGIE(data):

  # already sort data by the length in a decreasing order
  input_imgs, output_imgs, caps, cap_len = data['input_imgs'], data['output_imgs'], data['caps'], data['cap_len']

  input_imgs_list, output_imgs_list = [], []
  for i in range(len(input_imgs)):
    if cfg.CUDA:
      input_imgs_list.append(Variable(input_imgs[i]).cuda())
      output_imgs_list.append(Variable(output_imgs[i]).cuda())
    else:
      input_imgs_list.append(Variable(input_imgs[i]))
      output_imgs_list.append(Variable(output_imgs[i]))

  caps = caps.squeeze()

  if cfg.CUDA:
    caps = Variable(caps).cuda()
    cap_len = Variable(cap_len).cuda()
  else:
    caps = Variable(caps)
    cap_len = Variable(cap_len)

  return [input_imgs_list, output_imgs_list, caps, cap_len]
...
class TextDataset(data.Dataset):
  ...
  def get_caption(self, sent_ix):
    ...
    if num_words <= cfg.TEXT.WORDS_NUM:
      ...
      # 为啥长度超过了之后可以随意shuffle然后取前几个？不会破坏句子结构吗
